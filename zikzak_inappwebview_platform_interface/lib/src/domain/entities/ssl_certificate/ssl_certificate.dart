...
// SslCertificate carries custom platform-dependent deserialization (iOS
// parses the X509Certificate data; Android reads the map fields directly)
// that Zorphy value objects cannot express. Plain Dart class with an
// identical public API and wire format to the old codegen.

import '../../../util.dart';
import '../../../x509_certificate/x509_certificate.dart';
import '../../../x509_certificate/asn1_distinguished_names.dart';

import '../ssl_certificate_dname/ssl_certificate_dname.dart';
...
class SslCertificate {
  ...
  static SslCertificate? fromMap(Map<String, dynamic>? map) {
    ...
    return SslCertificate(
      issuedBy: map["issuedBy"] != null
          ? SslCertificateDName.fromJson(
              map["issuedBy"]!.cast<String, dynamic>(),
            )
          : null,
      issuedTo: map["issuedTo"] != null
          ? SslCertificateDName.fromJson(
