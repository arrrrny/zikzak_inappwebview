...
final _coreCheckerObjectConstructor = const TypeChecker.fromRuntime(
  ExchangeableObjectConstructor,
);
final _coreCheckerObjectProperty = const TypeChecker.fromRuntime(
  ExchangeableObjectProperty,
);
final _coreCheckerObjectMethod = const TypeChecker.fromRuntime(
...
final _coreCheckerSupportedPlatforms = const TypeChecker.fromRuntime(
  SupportedPlatforms,
);
...
class ExchangeableObjectGenerator
  ...
  String generateForAnnotatedElement(
    ...
    classBuffer.write(
      '${(element as ClassElement).isAbstract ? 'abstract ' : ''}class $extClassName',
    );
    ...
    for (final entry in fieldEntriesSorted) {
      ...
      // extract the type from the analyzer's resolved type
      var typeString = fieldElement.type.toString().replaceFirst("_", "");

      // When the field's type is also generated by this generator (e.g.
      // PrintJobColorMode), the analyzer can't resolve it and returns
      // "InvalidType". Fall back to the AST source text which preserves
      // the original type name.
      if (typeString == "InvalidType") {
        var parsed =
            fieldElement.session?.getParsedLibraryByElement(
                  fieldElement.library,
                )
                as ParsedLibraryResult?;
        var declaration = parsed?.getElementDeclaration(fieldElement);
        var parent = declaration?.node.parent;
        // The declaration's grandparent is FieldDeclaration, its child
        // VariableDeclarationList has the type annotation.
        if (parent is VariableDeclarationList && parent.type != null) {
          typeString = parent.type.toString();
        }
      }

      classBuffer.write("$typeString ");
    ...
    if (hasCustomConstructor) {
      ParsedLibraryResult parsed =
          visitor.constructor.session?.getParsedLibraryByElement(
                visitor.constructor.library,
              )
              as ParsedLibraryResult;
      final constructorBody = parsed
          .getElementDeclaration(visitor.constructor)
          ?.node;
        ...
        if (!fieldElement.isPrivate &&
          ...
          if (customDeserializer != null) {
            final deserializerClassName =
                (customDeserializer as dynamic).enclosingElement3?.name
                    as String?;
    ...
    for (final entry in methodEntriesSorted) {
      ...
      ParsedLibraryResult parsed =
          methodElement.session?.getParsedLibraryByElement(
                methodElement.library,
              )
              as ParsedLibraryResult;
      ...
      for (final fieldElement in fieldElements) {
        if (!fieldElement.isPrivate &&
          ...
          if (customSerializer != null) {
            final serializerClassName =
                (customSerializer as dynamic).enclosingElement3?.name
                    as String?;
