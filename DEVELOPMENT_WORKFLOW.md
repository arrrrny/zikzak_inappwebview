...
- **`development`** - Development branch (default). All PRs merge here first.

## Workflow

### 1. Development & PRs

All pull requests should target the **`development`** branch (this is now the default).
...
# Create PR targeting development (automatic default)
```

### 2. Testing on Dev Branch

After merging PRs to `development`:
...
Once testing is complete on `development`:
...
git merge development --no-ff -m "Merge dev to master: [describe changes]"
...
All open PRs have been updated to target `development`:
...
3. Confirm merge to development
...
- **Default Branch**: GitHub PRs will automatically target `development`
