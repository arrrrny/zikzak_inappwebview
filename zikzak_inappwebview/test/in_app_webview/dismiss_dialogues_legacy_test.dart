...
void main() {
  group('legacyDismissDialogues (U3-U8 characterization)', () {
    ...
    test(
      'legacyDismissDialoguesJs removes elements whose position is fixed or sticky',
      () {
        final js = legacyDismissDialoguesJs();
        expect(
          js,
          contains("document.querySelectorAll('*')"),
          reason: 'must scan every element',
        );
        expect(
          js,
          contains('window.getComputedStyle(el)'),
    ...
    test(
      'legacyDismissDialoguesJs resets overflow and margin to empty on documentElement and body',
      () {
        final js = legacyDismissDialoguesJs();
    ...
    test(
      'runLegacyDismissDialogues injects the script exactly 3 times with the 800ms retry delay',
      () async {
        ...
        expect(
          platform.evaluatedSources.length,
          3,
          reason: 'the legacy loop attempts three times',
        );
        expect(
          legacyDismissRetryDelay,
    ...
    test(
      'shouldDismissDialogues is false unless dismissDialogues is explicitly true',
      () {
    ...
    test(
      'removal errors are swallowed (inner catch + driver try/catch) and never propagate',
      () async {
        // The emitted script tolerates per-element DOM errors.
        expect(
          legacyDismissDialoguesJs(),
          contains('catch(e)'),
    ...
    test(
      'legacyDismissDialoguesJs traverses only the top-level document (no iframe/frames)',
      () {
        final js = legacyDismissDialoguesJs();
        expect(
          js,
