...
void main() {
  ...
  group('CookieController delegates to CookieManager (U46-U65)', () {
    test(
      'U46 getCookies without URL defers to getUrl() and passes controller ctx',
      () async {
    ...
    test(
      'U47 getCookies with explicit URL uses it and ignores current URL',
      () async {
        final fake = FakePlatformInAppWebViewController()
          ..nextUrl = WebUri(_currentUrl);
        final controller = _controller(fake);
    ...
    test(
      'U48 getCookies with no URL available returns [] (graceful)',
      () async {
    ...
    test(
      'U50 getCookie with explicit URL uses it and ignores current URL',
      () async {
        final fake = FakePlatformInAppWebViewController()
          ..nextUrl = WebUri(_currentUrl);
        final controller = _controller(fake);
    ...
    test(
      'U51 getCookie with no URL available returns null (graceful)',
      () async {
        final fake = FakePlatformInAppWebViewController()..nextUrl = null;
        final controller = _controller(fake);
        final cookieFake = FakePlatformCookieManager();
    ...
    test(
      'U53 setCookie with explicit URL uses it and ignores current URL',
      () async {
        final fake = FakePlatformInAppWebViewController()
          ..nextUrl = WebUri(_currentUrl);
        final controller = _controller(fake);
    ...
    test(
      'U54 setCookie with no URL available returns false (graceful)',
      () async {
        final fake = FakePlatformInAppWebViewController()..nextUrl = null;
        final controller = _controller(fake);
        final cookieFake = FakePlatformCookieManager();
    ...
    test(
      'U56 deleteCookie with explicit URL uses it and ignores current URL',
      () async {
        final fake = FakePlatformInAppWebViewController()
          ..nextUrl = WebUri(_currentUrl);
        final controller = _controller(fake);
    ...
    test(
      'U57 deleteCookie with no URL available returns false (graceful)',
      () async {
        final fake = FakePlatformInAppWebViewController()..nextUrl = null;
        final controller = _controller(fake);
        final cookieFake = FakePlatformCookieManager();
    ...
    test(
      'U59 deleteCookies with explicit URL uses it and ignores current URL',
      () async {
        final fake = FakePlatformInAppWebViewController()
          ..nextUrl = WebUri(_currentUrl);
        final controller = _controller(fake);
    ...
    test(
      'U60 deleteCookies with no URL available returns false (graceful)',
      () async {
        final fake = FakePlatformInAppWebViewController()..nextUrl = null;
        final controller = _controller(fake);
        final cookieFake = FakePlatformCookieManager();
    ...
    test(
      'U61 getAllCookies reads the shared store globally (not URL scoped)',
      () async {
        final fake = FakePlatformInAppWebViewController()
          ..nextUrl = WebUri(_currentUrl);
        final controller = _controller(fake);
    ...
    test(
      'U62 deleteAllCookies delegates to the shared store globally',
      () async {
        final fake = FakePlatformInAppWebViewController()
          ..nextUrl = WebUri(_currentUrl);
        final controller = _controller(fake);
    ...
    test(
      'U63 removeSessionCookies delegates to the shared store globally',
      () async {
        final fake = FakePlatformInAppWebViewController()
          ..nextUrl = WebUri(_currentUrl);
        final controller = _controller(fake);
    ...
    test(
      'U64 injected CookieManager override is used, singleton untouched',
      () async {
        final fake = FakePlatformInAppWebViewController()
          ..nextUrl = WebUri(_currentUrl);
        final controller = _controller(fake);
