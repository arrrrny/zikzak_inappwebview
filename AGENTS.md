...
- **CI exists and gates `master`.** `.github/workflows/ci.yml` runs a 16-job matrix on every push to `master` and every PR into it: `flutter analyze` and `flutter test` for `platform_interface`, `ios`, `web`, `macos`, `windows`, `linux` and `zikzak_inappwebview` (`web` with `--platform chrome`), plus a two-runner `build-ios` job (`macos-14` and `macos-15`) that compiles the iOS Swift sources by building the example app from this checkout. `zikzak_inappwebview_android` is the only published package still outside the matrix. There is no Makefile or Jenkinsfile. The authoritative gate remains running `flutter analyze --no-fatal-infos` + `flutter test` yourself in each affected package before you push.
