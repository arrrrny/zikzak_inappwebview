...
//  zikzak_inappwebview
//
//  macOS port of the iOS WebMessagePort type. See issue #197.
//
...
public class WebMessagePort: NSObject {
    ...
    public func postMessage(message: WebMessage, completionHandler: ((Any?) -> Void)? = nil)
        throws
    {
        if isClosed || isTransferred {
            throw NSError(domain: "Port is already closed or transferred", code: 0)
        ...
        if let webMessageChannel = webMessageChannel, let webView = webMessageChannel.webView {
            ...
            let source = """
                ...
                """
            webView.evaluateJavaScript(source, completionHandler: { (_, _) in
                completionHandler?(nil)
            })
        } else {
            completionHandler?(nil)
